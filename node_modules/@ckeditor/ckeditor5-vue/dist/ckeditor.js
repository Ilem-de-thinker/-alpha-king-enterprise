...
import { computed, createBlock, defineComponent, getCurrentInstance, markRaw, mergeModels, mergeProps, onBeforeUnmount, onMounted, openBlock, ref, resolveDynamicComponent, shallowReadonly, toValue, unref, useModel, version, watch, watchEffect } from "vue";
...
function useEditorElementDefinition({ Editor, config, defaultElementName }) {
	return computed(() => {
		...
		if (!isClassicEditor(toValue(Editor))) {
...
var EditorElement_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose }) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(resolveDynamicComponent(definition.value.name), mergeProps({
				ref_key: "elementRef",
				ref: elementRef
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		onMounted(async () => {
			...
			try {
				...
				const editor = await (supports.elementConfigAttachment ? Constructor.create(assignElementToEditorConfig(Constructor, domElement, editorConfig)) : Constructor.create(domElement, editorConfig));
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(EditorElement_default, {
				ref_key: "editorElementRef",
				ref: editorElementRef,
				definition: unref(elementDefinition)
			}, null, 8, ["definition"]);
		};
	}
});
