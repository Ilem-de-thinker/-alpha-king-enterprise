<template>
  <div>
    <div class="page-header">
      ...
      <router-link to="/posts/create" class="btn-primary"><i class="fa fa-plus"></i> New Post</router-link>
    </div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="p in posts" :key="p.id">
            ...
            <td class="actions">
              <router-link :to="`/posts/${p.id}/edit`" class="btn-sm" title="Edit"><i class="fa fa-pencil"></i></router-link>
              <button class="btn-sm danger" @click="remove(p.id)" title="Delete"><i class="fa fa-trash"></i></button>
            </td>
          </tr>
          <tr v-if="!posts.length"><td colspan="5" class="empty">No posts yet. <router-link to="/posts/create">Create one</router-link></td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
onMounted(async () => {
  try {
    ...
    posts.value = Array.isArray(data) ? data : data.posts || data.data || []
  } catch (e) { posts.value = [] }
})
