<template>
  <div>
    <div class="page-header">
      <div><h3>About Features</h3><p>Manage about page feature bullet points</p></div>
      <button class="btn-primary" @click="showForm = true; editItem = null"><i class="fa fa-plus"></i> Add</button>
    </div>
    <div class="table-card">
      <table>
        <thead><tr><th>Order</th><th>Icon</th><th>Text</th><th>Actions</th></tr></thead>
        <tbody>
          <tr v-for="f in items" :key="f.id">
            <td>{{ f.order }}</td><td><code>{{ f.icon }}</code></td><td><strong>{{ f.text }}</strong></td>
            <td class="actions">
              <button class="btn-sm" @click="edit(f)"><i class="fa fa-pencil"></i></button>
              <button class="btn-sm danger" @click="remove(f.id)"><i class="fa fa-trash"></i></button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} Feature</h4>
        <div class="form-group"><label>Icon</label><input v-model="form.icon" placeholder="fa-check" /></div>
        <div class="form-group"><label>Text</label><input v-model="form.text" /></div>
...
const form = ref({ icon: 'fa-check', text: '', order: 0 })
const formError = ref('')
const saving = ref(false)

onMounted(load)
async function load() { try { const { data } = await api.get('/about/features'); items.value = Array.isArray(data) ? data : [] } catch { items.value = [] } }
function edit(f) { editItem.value = f; form.value = { icon: f.icon, text: f.text, order: f.order || 0 }; showForm.value = true }
async function save() {
  formError.value = ''; saving.value = true
  try {
    if (editItem.value) await api.put("/about/features/" + editItem.value.id, form.value)
    else await api.post('/about/features', form.value)
    showForm.value = false; editItem.value = null; form.value = { icon: 'fa-check', text: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete("/about/features/" + id); await load() }
</script>
