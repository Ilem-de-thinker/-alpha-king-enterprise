<template>
  <div>
    <div class="page-header">
      <div><h3>Trust Bar</h3><p>Scrolling trust indicators</p></div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="t in items" :key="t.id">
            <td>{{ t.order }}</td><td><code>{{ t.icon }}</code></td><td><strong>{{ t.text }}</strong></td>
            <td class="actions">
              <button class="btn-sm" @click="edit(t)"><i class="fa fa-pencil"></i></button>
              <button class="btn-sm danger" @click="remove(t.id)"><i class="fa fa-trash"></i></button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} Trust Item</h4>
        <div class="form-group"><label>Icon (Font Awesome class)</label><input v-model="form.icon" placeholder="fa-shield" /></div>
        <div class="form-group"><label>Text</label><input v-model="form.text" placeholder="Security First" /></div>
...
const form = ref({ icon: '', text: '', order: 0 })
const formError = ref('')
const saving = ref(false)

onMounted(load)
async function load() { try { const { data } = await api.get('/trust-bar'); items.value = Array.isArray(data) ? data : [] } catch { items.value = [] } }
...
async function save() {
  ...
  try {
    if (editItem.value) await api.put(`/trust-bar/${editItem.value.id}`, form.value)
    else await api.post('/trust-bar', form.value)
    showForm.value = false; editItem.value = null; form.value = { icon: '', text: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete(`/trust-bar/${id}`); await load() }
</script>
