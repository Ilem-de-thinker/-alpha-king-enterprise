<template>
  <div>
    <div class="page-header">
      <div><h3>Counters</h3><p>Statistics counters section</p></div>
      <button class="btn-primary" @click="showForm = true; editItem = null"><i class="fa fa-plus"></i> Add</button>
    </div>
    <div class="table-card">
      <table>
        <thead><tr><th>Order</th><th>Icon</th><th>Number</th><th>Suffix</th><th>Label</th><th>Actions</th></tr></thead>
        <tbody>
          <tr v-for="c in items" :key="c.id">
            <td>{{ c.order }}</td><td><code>{{ c.icon }}</code></td><td><strong>{{ c.number }}</strong></td><td>{{ c.suffix }}</td><td>{{ c.label }}</td>
            <td class="actions">
              <button class="btn-sm" @click="edit(c)"><i class="fa fa-pencil"></i></button>
              <button class="btn-sm danger" @click="remove(c.id)"><i class="fa fa-trash"></i></button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} Counter</h4>
        <div class="form-group"><label>Icon (Font Awesome)</label><input v-model="form.icon" placeholder="fa-users" /></div>
        <div class="form-row">
          <div class="form-group"><label>Number</label><input v-model="form.number" type="number" /></div>
          <div class="form-group"><label>Suffix</label><input v-model="form.suffix" placeholder="+" /></div>
        </div>
        <div class="form-group"><label>Label</label><input v-model="form.label" placeholder="Projects Delivered" /></div>
...
const form = ref({ icon: '', number: 0, suffix: '+', label: '', order: 0 })
const formError = ref('')
const saving = ref(false)

onMounted(load)
async function load() { try { const { data } = await api.get('/counters'); items.value = Array.isArray(data) ? data : [] } catch { items.value = [] } }
...
async function save() {
  ...
  try {
    if (editItem.value) await api.put("/counters/" + editItem.value.id, form.value)
    else await api.post('/counters', form.value)
    showForm.value = false; editItem.value = null; form.value = { icon: '', number: 0, suffix: '+', label: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete("/counters/" + id); await load() }
</script>
