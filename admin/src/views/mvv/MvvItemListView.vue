<template>
  <div>
    <div class="page-header">
      <div><h3>MVV Items</h3><p>Manage Vision, Mission, Values cards</p></div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="m in items" :key="m.id">
            <td>{{ m.order }}</td><td><code>{{ m.icon }}</code></td><td><strong>{{ m.title }}</strong></td>
            <td class="actions">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} MVV Item</h4>
        <div class="form-group"><label>Icon (class)</label><input v-model="form.icon" placeholder="fa-eye" /></div>
...
const form = ref({ icon: '', title: '', description: '', order: 0 })
const formError = ref('')
const saving = ref(false)

onMounted(load)
async function load() { try { const { data } = await api.get('/mvv/items'); items.value = Array.isArray(data) ? data : [] } catch { items.value = [] } }
...
async function save() {
  ...
  try {
    if (editItem.value) await api.put("/mvv/items/" + editItem.value.id, form.value)
    else await api.post('/mvv/items', form.value)
    showForm.value = false; editItem.value = null; form.value = { icon: '', title: '', description: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete("/mvv/items/" + id); await load() }
</script>
<style scoped>
...
td { border-top: 1px solid #e2e8f0; code { background: #f0f4f8; padding: 2px 6px; border-radius: 4px; font-size: 12px; } }
