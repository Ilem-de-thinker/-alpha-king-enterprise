<template>
  <div>
    <div class="page-header"><h3>Newsletter Section</h3><p>Edit the newsletter signup section</p></div>
    <div class="form-card">
      <form @submit.prevent="save">
        ...
        <div class="form-row">
          <div class="form-group"><label>Icon (class)</label><input v-model="form.icon" /></div>
          <div class="form-group"><label>Placeholder</label><input v-model="form.placeholder" /></div>
        </div>
...
const form = ref({ title: '', description: '', icon: 'fa-envelope-o', placeholder: '', buttonText: '' })
const saving = ref(false)
const error = ref('')

onMounted(async () => { try { const { data } = await api.get('/newsletter-section'); Object.assign(form.value, data) } catch {} })
async function save() {
  error.value = ''; saving.value = true
  try { await api.put('/newsletter-section', form.value) }
