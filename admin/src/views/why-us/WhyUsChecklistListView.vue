<template>
  <div>
    <div class="page-header">
      <div><h3>Why Us Checklist</h3><p>Manage checklist items</p></div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="c in items" :key="c.id">
            <td>{{ c.order }}</td><td><code>{{ c.icon }}</code></td><td><strong>{{ c.title }}</strong></td>
            <td class="actions">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} Checklist Item</h4>
        <div class="form-group"><label>Icon</label><input v-model="form.icon" placeholder="fa-check-circle" /></div>
...
const form = ref({ icon: 'fa-check-circle', title: '', description: '', order: 0 })
const formError = ref('')
const saving = ref(false)

onMounted(load)
async function load() { try { const { data } = await api.get('/why-us/checklist'); items.value = Array.isArray(data) ? data : [] } catch { items.value = [] } }
function edit(c) { editItem.value = c; form.value = { icon: c.icon, title: c.title, description: c.description || '', order: c.order || 0 }; showForm.value = true }
async function save() {
  formError.value = ''; saving.value = true
  try {
    if (editItem.value) await api.put("/why-us/checklist/" + editItem.value.id, form.value)
    else await api.post('/why-us/checklist', form.value)
    showForm.value = false; editItem.value = null; form.value = { icon: 'fa-check-circle', title: '', description: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete("/why-us/checklist/" + id); await load() }
</script>
