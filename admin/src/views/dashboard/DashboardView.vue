  ...
  <div>
    ...
    <div class="quick-links">
      ...
      <div class="links-grid">
        <router-link to="/posts/create" class="quick-link"><i class="fa fa-plus"></i> New Post</router-link>
        <router-link to="/contacts" class="quick-link"><i class="fa fa-envelope"></i> View Messages</router-link>
        <router-link to="/comments" class="quick-link"><i class="fa fa-comments"></i> Moderate Comments</router-link>
      </div>
    </div>
  </div>
</template>
...
onMounted(async () => {
  try {
    const [posts, comments, categories, services, team, projects, contacts, newsletter] = await Promise.all([
      api.get('/posts?limit=1'), api.get('/posts/default/comments?limit=1'),
      api.get('/categories'), api.get('/services'),
      api.get('/team'), api.get('/projects'),
      api.get('/contact'), api.get('/newsletter')
    ])
    stats.value[0].count = posts.data.total || posts.data.length || 0
    stats.value[1].count = comments.data.total || comments.data.length || 0
    stats.value[2].count = categories.data.length || 0
    stats.value[3].count = services.data.length || 0
    stats.value[4].count = team.data.length || 0
    stats.value[5].count = projects.data.length || 0
    stats.value[6].count = contacts.data.length || 0
    stats.value[7].count = newsletter.data.length || 0
  } catch {}
})
</script>
