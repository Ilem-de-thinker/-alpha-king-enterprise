<template>
  <div>
    ...
    <template v-if="loading">
      <section class="ve-page-hero" style="background-image:url(/img/bg-img/1.jpg);">
        <div class="ve-page-hero-overlay"></div>
        <div class="container ve-page-hero-content">
          <ul class="ve-breadcrumb"><li><router-link to="/">Home</router-link></li><li class="active">...</li></ul>
            ...
            <div class="col-12 col-lg-7">
              <div class="ve-about-text">
                ...
                <div class="ve-about-features">
                  <div class="ve-af-item" v-for="f in []" :key="f.id">
                    <i :class="'fa ' + (f.icon || 'fa-check')"></i> {{ f.text }}
                  </div>
                </div>
                <router-link :to="about.ctaUrl || '/services'" class="ve-btn-primary mt-30">{{ about.ctaText }}</router-link>
              </div>
            </div>
          </div>
        </div>
...
onMounted(async () => {
  try {
    const [aRes, mRes, miRes, tRes] = await Promise.all([
      api.get('/about'), api.get('/mvv'),
