<template>
  <div>
    <div class="page-header">
      <div><h3>Projects</h3><p>Manage featured projects</p></div>
      <button class="btn-primary" @click="openAdd"><i class="fa fa-plus"></i> Add</button>
    </div>
    <div class="table-card">
      <table>
        <thead><tr><th>Order</th><th>Title</th><th>Category</th><th>Actions</th></tr></thead>
        <tbody>
          <tr v-for="p in items" :key="p.id">
            ...
            <td class="actions">
              <button class="btn-sm" @click="edit(p)"><i class="fa fa-pencil"></i></button>
              <button class="btn-sm danger" @click="remove(p.id)"><i class="fa fa-trash"></i></button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} Project</h4>
        <div class="form-group"><label>Title *</label><input v-model="form.title" required /></div>
        <div class="form-group"><label>Description *</label><textarea v-model="form.description" rows="3" required></textarea></div>
        <div class="form-group"><label>Image</label><ImageUpload v-model="form.image" @file="imageFile = $event" /></div>
        <div class="form-group"><label>Category *</label><input v-model="form.category" required /></div>
        ...
        <div class="form-actions">
          ...
          <button class="btn-primary" @click="save" :disabled="saving || !form.title || !form.description || !form.category">{{ saving ? 'Saving...' : 'Save' }}</button>
        </div>
      </div>
    </div>
  </div>
...
const DEFAULT_FORM = { title: '', description: '', image: '', category: '', order: 0 }
const items = ref([])
...
async function load() {
  try { const { data } = await api.get('/projects/'); items.value = data.results || (Array.isArray(data) ? data : []) }
  catch { items.value = [] }
}
...
function edit(p) {
  editItem.value = p
  form.value = { title: p.title, description: p.description || '', image: p.image || '', category: p.category || '', order: p.order || 0 }
  imageFile.value = null
  formError.value = ''
  showForm.value = true
}
...
async function save() {
  ...
  try {
    ...
    if (editItem.value) await api.put(`/projects/${editItem.value.id}/`, fd)
...
async function remove(id) { if (!confirm('Delete this project?')) return; await api.delete(`/projects/${id}/`); await load() }
</script>
...
.badge { background: rgba(0,123,255,0.1); color: #007bff; font-size: 12px; font-weight: 700; padding: 4px 12px; border-radius: 20px; }
