...
const form = ref({ title: '', description: '', image: '', category: '', order: 0 })
const formError = ref('')
const saving = ref(false)

onMounted(load)
async function load() { try { const { data } = await api.get('/projects'); items.value = data.results || (Array.isArray(data) ? data : []) } catch { items.value = [] } }

function edit(p) { editItem.value = p; form.value = { title: p.title, description: p.description || '', image: p.image || '', category: p.category || '', order: p.order || 0 }; showForm.value = true }

async function save() {
  formError.value = ''; saving.value = true
  try {
    if (editItem.value) await api.put(`/projects/${editItem.value.id}`, form.value)
    else await api.post('/projects', form.value)
    showForm.value = false; editItem.value = null; form.value = { title: '', description: '', image: '', category: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete(`/projects/${id}`); await load() }
</script>
