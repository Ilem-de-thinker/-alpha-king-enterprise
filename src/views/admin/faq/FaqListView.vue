<template>
  <div>
    <div class="page-header">
      <div><h3>FAQ</h3><p>Manage frequently asked questions</p></div>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} FAQ</h4>
        <div class="form-group"><label>Question</label><input v-model="form.question" /></div>
        <div class="form-group"><label>Answer</label><textarea v-model="form.answer" rows="4"></textarea></div>
...
async function load() { try { const { data } = await api.get('/faq'); items.value = data.results || (Array.isArray(data) ? data : []) } catch { items.value = [] } }

function edit(f) { editItem.value = f; form.value = { question: f.question, answer: f.answer || '', order: f.order || 0 }; showForm.value = true }

async function save() {
  formError.value = ''; saving.value = true
  try {
    if (editItem.value) await api.put(`/faq/${editItem.value.id}`, form.value)
    else await api.post('/faq', form.value)
    showForm.value = false; editItem.value = null; form.value = { question: '', answer: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete(`/faq/${id}`); await load() }
</script>
...
.modal-card { background: #fff; border-radius: 12px; padding: 32px; width: 100%; max-width: 560px; }
