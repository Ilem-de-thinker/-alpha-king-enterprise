      ...
      <form @submit.prevent="save">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <ImageUpload v-model="form.featuredImage" @file="featuredFile = $event" />
          </div>
          <div class="form-group">
            <label>Read Time *</label>
            <input v-model="form.readTime" placeholder="e.g. 6 min read" required />
          </div>
        </div>
        <div class="form-group">
...
const FORM_KEYS = ['title', 'slug', 'excerpt', 'content', 'featuredImage', 'category', 'tags', 'author', 'readTime']
...
async function save() {
  ...
  try {
    ...
    FORM_KEYS.forEach(k => {
      if (k === 'tags') {
        form.value.tags.forEach(t => fd.append('tags', t))
      } else if (k === 'featuredImage' && featuredFile.value) {
        fd.append('featuredImage', featuredFile.value)
      } else {
        fd.append(k, form.value[k] ?? '')
      }
    })
    if (isEdit.value) await api.put(`/posts/${postId.value}/`, fd)
    else await api.post('/posts/', fd)
    router.push('/admin/posts')
  } catch (e) {
    error.value = e.response?.data?.error || e.response?.data?.message || 'Failed to save'
...
.form-group label { font-size: 12px; font-weight: 700; color: #0d1b2a; text-transform: uppercase; letter-spacing: 0.5px; }
.form-group input, .form-group select, .form-group textarea {
  border: 2px solid #e2e8f0; border-radius: 8px; padding: 10px 14px; font-size: 14px;
  font-family: 'DM Sans', sans-serif; color: #0d1b2a; outline: none; transition: border-color 0.2s;
}
.form-group input:focus, .form-group select:focus, .form-group textarea:focus { border-color: #007bff; box-shadow: 0 0 0 3px rgba(0,123,255,0.1); }
...
.btn-ghost { display: inline-flex; align-items: center; padding: 12px 24px; border-radius: 8px; font-weight: 700; font-size: 14px; border: 2px solid #e2e8f0; color: #4a5568; background: #fff; transition: all 0.2s; }
