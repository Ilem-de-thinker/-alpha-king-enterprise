<template>
  <div>
    <div class="page-header">
      <div><h3>Hero Stats</h3><p>Statistics displayed in the hero section</p></div>
      <button class="btn-primary" @click="showForm = true; editItem = null"><i class="fa fa-plus"></i> Add</button>
    </div>
    <div class="table-card">
      <table>
        <thead><tr><th>Order</th><th>Value</th><th>Label</th><th>Actions</th></tr></thead>
        <tbody>
          <tr v-for="s in items" :key="s.id">
            <td>{{ s.order }}</td><td><strong>{{ s.value }}</strong></td><td>{{ s.label }}</td>
            <td class="actions">
              <button class="btn-sm" @click="edit(s)"><i class="fa fa-pencil"></i></button>
              <button class="btn-sm danger" @click="remove(s.id)"><i class="fa fa-trash"></i></button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} Stat</h4>
        <div class="form-group"><label>Value</label><input v-model="form.value" placeholder="50+" /></div>
...
const form = ref({ value: '', label: '', order: 0 })
const formError = ref('')
const saving = ref(false)

onMounted(load)
async function load() { try { const { data } = await api.get('/hero-stats'); items.value = data.results || (Array.isArray(data) ? data : []) } catch { items.value = [] } }
function edit(s) { editItem.value = s; form.value = { value: s.value, label: s.label, order: s.order || 0 }; showForm.value = true }
async function save() {
  formError.value = ''; saving.value = true
  try {
    if (editItem.value) await api.put(`/hero-stats/${editItem.value.id}`, form.value)
    else await api.post('/hero-stats', form.value)
    showForm.value = false; editItem.value = null; form.value = { value: '', label: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete(`/hero-stats/${id}`); await load() }
</script>
