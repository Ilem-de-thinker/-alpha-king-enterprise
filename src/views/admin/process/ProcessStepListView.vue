<template>
  <div>
    <div class="page-header">
      <div><h3>Process Steps</h3><p>Manage the process steps on services page</p></div>
      <button class="btn-primary" @click="showForm = true; editItem = null"><i class="fa fa-plus"></i> Add</button>
    </div>
    <div class="table-card">
      <table>
        <thead><tr><th>Order</th><th>Step</th><th>Title</th><th>Actions</th></tr></thead>
        <tbody>
          <tr v-for="s in items" :key="s.id">
            <td>{{ s.order }}</td><td><strong>{{ s.step }}</strong></td><td>{{ s.title }}</td>
            <td class="actions">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} Process Step</h4>
        <div class="form-group"><label>Step Number</label><input v-model="form.step" placeholder="01" /></div>
...
const form = ref({ step: '', title: '', description: '', order: 0 })
const formError = ref('')
...
async function load() { try { const { data } = await api.get('/process'); items.value = data.results || (Array.isArray(data) ? data : []) } catch { items.value = [] } }
function edit(s) { editItem.value = s; form.value = { step: s.step, title: s.title, description: s.description || '', order: s.order || 0 }; showForm.value = true }
async function save() {
  formError.value = ''; saving.value = true
  try {
    if (editItem.value) await api.put("/process/" + editItem.value.id, form.value)
    else await api.post('/process', form.value)
    showForm.value = false; editItem.value = null; form.value = { step: '', title: '', description: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete("/process/" + id); await load() }
</script>
<style scoped>
