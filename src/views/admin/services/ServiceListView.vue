<template>
  <div>
    <div class="page-header">
      <div><h3>Services</h3><p>Manage service listings</p></div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="s in items" :key="s.id">
            ...
            <td>{{ s.slug }}</td>
            <td class="actions">
              <button class="btn-sm" @click="edit(s)" title="Edit"><i class="fa fa-pencil"></i></button>
              <button class="btn-sm danger" @click="remove(s.id)" title="Delete"><i class="fa fa-trash"></i></button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-card">
        <h4>{{ editItem ? 'Edit' : 'New' }} Service</h4>
        <div class="form-group"><label>Title</label><input v-model="form.title" @input="form.slug = form.title.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'')" /></div>
        <div class="form-group"><label>Slug</label><input v-model="form.slug" /></div>
        <div class="form-group"><label>Icon (class name)</label><input v-model="form.icon" placeholder="icon-profits" /></div>
...
const form = ref({ title: '', slug: '', icon: '', description: '', order: 0 })
const formError = ref('')
const saving = ref(false)

onMounted(load)
async function load() { try { const { data } = await api.get('/services'); items.value = data.results || (Array.isArray(data) ? data : []) } catch { items.value = [] } }

function edit(s) { editItem.value = s; form.value = { title: s.title, slug: s.slug, icon: s.icon || '', description: s.description || '', order: s.order || 0 }; showForm.value = true }
...
async function save() {
  ...
  try {
    ...
    showForm.value = false; editItem.value = null; form.value = { title: '', slug: '', icon: '', description: '', order: 0 }; await load()
  } catch (e) { formError.value = e.response?.data?.message || 'Error saving' }
  finally { saving.value = false }
}
async function remove(id) { if (!confirm('Delete?')) return; await api.delete(`/services/${id}`); await load() }
</script>
